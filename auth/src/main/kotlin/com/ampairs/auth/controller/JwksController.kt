...
class JwksController(
     ...
     * Catches all errors to prevent JWT verifiers from receiving non-JSON errors.
     */
    @GetMapping("/.well-known/jwks.json")
    fun getJwks(): ResponseEntity<Map<String, Any>> {
        return try {
            val jwkSet = rsaKeyManager.getJwkSet()
            logger.debug("Serving JWKS with {} keys", (jwkSet["keys"] as? List<*>)?.size ?: 0)
            ResponseEntity.ok()
            ...
            ResponseEntity.internalServerError()
                .body(mapOf("keys" to emptyList<Map<String, Any>>()))
        }
    }
    ...
    /**
     * Returns specific public key in JWK format for the given kid.
     * Returns 404 if key not found. Catches errors to stay JSON-safe for verifiers.
     */
    @GetMapping("/{keyId}")
    ...
    fun getKeyStatistics(): ApiResponse<Map<String, Any>> {
        return ApiResponse.success(rsaKeyManager.getKeyStatistics())
    }
    ...
    fun getKeySystemHealth(): ApiResponse<Map<String, Any>> {
        ...
        return ApiResponse.success(mapOf<String, Any>(
            "status" to "healthy",
    ...
    data class KeyRotationRequest(val reason: String? = null)
}
