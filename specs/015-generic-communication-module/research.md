...
**Decision**: A `TransactionalEventListener` in `communication` subscribes with `@EventListener @Async` to the existing `event`-module domain events — currently **`InvoiceCreatedEvent` + `OrderCreatedEvent`** (both implemented) — exactly as `WorkspaceEventListener` does. There is **no payment domain event today** (the `event` module has Customer/Invoice/Order/Member/Product only); a `PaymentReceivedEvent` would have to be added to `event`/`payment` first before a payment trigger can be wired, so payment-received messaging is out of the current scope. Each event type maps (via a small workspace-configurable binding) to a template code + channel set; the listener resolves the customer (audience = the event's entity), renders, and dispatches. `communication` depends on `:event` for the event classes.
