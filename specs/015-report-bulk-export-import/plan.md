...
**Testing**: Backend — JUnit + Testcontainers (`./gradlew :report:test`). Per the constitution's **≥90% API-endpoint coverage** gate, contract/integration tests for the **new endpoints** are **REQUIRED**, not optional: templates `/sync` (T034a) and the SERVER export/import jobs + idempotency + tenant scope + 50k non-blocking smoke (T048a, T050a). App — compile gate on all 3 targets; unit tests for format writers/parsers, the descriptor↔DTO mapping, and the unsynced-local conflict policy (optional). Document any coverage gaps here per the constitution.
...
| Tenant context at controller | Template + job controllers set/clear `TenantContextHolder`; services never do. Jobs extend `OwnableBaseDomain`. **Async worker**: `DataJobWorker` runs outside a request, so it establishes tenant scope from `DataJob.ownerId` in try/finally before any repo access — the sanctioned manual override (constitution IV), not a service setting its own scope. |
| Testing & quality gates | New endpoints (templates `/sync`, exports/imports jobs) carry **required** contract/integration tests (T034a, T048a, T050a) to meet the ≥90% endpoint-coverage gate; CI must be green before merge. |
