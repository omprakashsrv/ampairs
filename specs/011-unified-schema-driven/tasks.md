...
- [ ] T005 [P] `BE:` `FormField` entity (`OwnableBaseDomain`, `Instant`, `active` soft-delete, **mandatory `section_uid` FK**, all columns per data-model.md) in `form/.../domain/model/FormField.kt`.
- [ ] T006 [P] `BE:` `FormSection` entity (first-class, `active`, order, visible) + the `FormSchema` aggregate-root domain type (per workspace+entityType; assembles ordered sections→fields; holds the invariant checks) in `form/.../domain/model/`.
...
- [ ] T017 [P] `BE:` Contract test (both feeds): `sections/sync` + `fields/sync` GET include soft-deleted rows with stable `(updatedAt, uid)` paging (no boundary skip on equal timestamps), POST upserts + honors `active=false`, snake_case params, `ApiResponse<PageResponse>` shape — `form/src/test/.../ConfigSyncContractTest.kt`.
...
- [ ] T022 `APP:` `ConfigApi`(+`ConfigApiImpl`) — `getSectionsSync`/`pushSections`, `getFieldsSync`/`pushFields`, `getConfigSchema`; remove the old `getFieldConfigsSync`/`getAttributeDefinitionsSync`. (Depends T019.)
- [ ] T023 `APP:` `ConfigRepository` local-only writes — `saveConfigSchema` validates the `FormSchema` aggregate (every field in a section; invariants) before writing rows `synced=false` + `markPendingPush(FORM)`; soft-delete = `active=false, synced=false`; `getConfigSchema` UI read retained; delete legacy `syncFormConfigs()` pull. (Depends T020, T022.)
- [ ] T024 `APP:` `FormSyncDelegate` drives BOTH feeds under one `SyncEntity.FORM` checkpoint — pull/push **sections before fields**; soft-delete aware; batches of `BATCH_SIZE` per feed; local-unsynced-wins; hard-delete on server-deleted; advance the single checkpoint to `max(updatedAt)` across both feeds after both drain; tolerate a dangling `sectionUid` (render in default group until the section arrives). (Depends T020, T022.)
...
- [ ] T057 [P] [US4] `BE:` Sync test — soft-deleted field appears in `fields/sync` and soft-deleted section in `sections/sync` pull feeds and round-trips via push (deletion reaches other devices). (Closes the documented known gap.)
...
- [ ] T071 [P] `APP:` Update `feature/form` docs + `.claude/skills/offline-sync` Form note (now full soft-delete round-trip; two feeds — sections + fields — under one `SyncEntity.FORM` checkpoint).
