...
- Q: With multiple connectors syncing the same entity into one workspace, how are records matched (each record has only one `refId`)? → A: Single `refId`, one owner — the entity's single `refId` is the identity key; the connector that first creates/owns a record owns its `refId`. A second connector reconciles to that record by business keys when possible, otherwise creates its own separate record. (Field mapping is defined per connector installation, so each connector has its own mapping.)
...
- **Unmapped field carries null/empty in the payload**: the server determines writable columns from the connector's stored mapping, so a null/empty value for an unmapped field is ignored and the existing server value is preserved (no data loss) — the failure mode the current full-upsert `/sync` would otherwise cause.
...
- **Multiple connectors syncing the same entity into one workspace**: identity is matched by the single `refId` owned by the first/owning connector; a second connector reconciles by business keys or keeps its own separate record, and never overwrites another connector's `refId`. If two connectors map the same field, the later write wins. Each connector still writes only its own mapped columns.
...
- **FR-018**: When the backend persists a connector-synced record, it MUST update only the mapped fields and MUST preserve the existing values of unmapped columns on that record (mapped-fields-only partial update). The set of writable columns MUST be derived from the connector's backend-persisted mapping for that entity — NOT inferred from which payload fields are non-null — so that an unmapped field can never overwrite existing data even if the incoming payload carries null or empty for it.
- **FR-018a**: The partial-update behavior MUST be applied on the connector-originated write path only. The existing global `/sync` upsert contract used by normal offline clients MUST remain unchanged (no field-mask or merge semantics imposed on non-connector sync).
- **FR-018b**: The backend MUST load the existing record (matched per FR-019) before applying a connector's mapped fields, and MUST leave fields outside the connector's mapping — including audit/system columns and fields owned by other writers — untouched.
- **FR-019**: The system MUST match incoming external records to existing Ampairs records by the entity's single stable external identifier (`refId`) within the workspace, so that repeated syncs update rather than duplicate. The connector that first creates/owns a record owns its `refId`; a second connector syncing the same entity MUST reconcile to that record by business keys when possible, and otherwise create its own separate record (it MUST NOT overwrite or collide on another connector's `refId`).
...
- **External Record Identity**: The entity's single `refId` holds the stable external identifier of the connector that owns that record; it is the match key for repeated syncs. A given record has exactly one owning connector's `refId`; other connectors reconcile by business keys or maintain their own separate records.
...
- **SC-004**: Across repeated syncs, zero duplicate Ampairs records are created from the same external record, and 100% of unmapped fields on updated records retain their prior values — including when a connector pushes records whose unmapped fields are null/empty (no data loss).
...
- **Partial-update mechanism**: The mapped-fields-only update on the connector write path reuses the project's existing load-existing-then-copy-mapped-fields merge pattern (as already implemented for the product `/sync` upsert), gated by the connector's backend-stored field mapping — rather than relying on payload nullness or a client-sent field mask.
