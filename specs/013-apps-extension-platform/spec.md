...
- **FR-019**: The system MUST match incoming external records to existing Ampairs records by a stable external identifier (the entity's `refId`) so that repeated syncs update rather than duplicate; the external-id match MUST be scoped to the workspace and connector.
