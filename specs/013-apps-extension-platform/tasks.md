...
- [X] T011 [FOUND] Module gating in `service/ConnectorCatalogueService.kt`: catalogue filtered by `InstalledModulesProvider.enabledModuleCodes()` (injected as `ObjectProvider` so connector-only test contexts still load). Connectors with a `requiredModule` only appear when that module is enabled. Tier (`requiredTier`) enforcement needs a current-tier SPI — left as metadata/follow-up.
...
- [X] T035 [P] [US3] Implement `ConnectorEntityWriter` per target module via `core.connector.AbstractRefIdEntityWriter` (refId/uid match → apply present columns → save; Principle IX). Done: customer, customer_group (customer module), product, product_group, product_category, **stock_balance** (product module), unit (unit module). Catalogue split `product_catalog` → `product_group`/`product_category`; product references target `*Id` fields. Complex/nested targets use the mapping DSL (`core.connector.DslEntityWriter`). **`stock_balance`** re-maps Tally's absolute closing balance onto the spec-014 multi-warehouse inventory model: match product → default warehouse → the product's **already-tracked** `InventoryItem` (opt-in, FR-004; never auto-creates) → idempotent, ledger-consistent `InventoryTransactionService.physicalCount` (only on a real change). Unit-tested in `ConnectorProductStockWriterTest`.
...
- [X] T040 [US3] [TEST] Docker-free unit tests for the data-integrity crux: `SparsePropertyApplierTest` (presence/coercion, omitted preserved, explicit-null clears) and `ConnectorSparseUpsertServiceTest` (per-row presence ∩ allowlist — row1≠row2 columns, out-of-allowlist dropped, unsupported entity rejected) — SC-004/FR-018. Per-row REQUIRES_NEW isolation via `ConnectorRowWriter` (FR-022). Testcontainers end-to-end + SC-008 checkpoint-resume still a follow-up.
