...
- [ ] T011 [BE] Retire `InventoryConfig`: delete `InventoryConfig.kt`, `InventoryConfigService`, `InventoryConfigRepository`, config DTOs; replace all reads with `settingService.getBoolean("inventory", key)`.
...
- [ ] T023 [BE] [US1] Wire **order** service to call `InventoryStockService.applySale(ORDER, orderUid, lines)` on order-confirm and `reverseSale(ORDER, …)` on order-cancel (R3 default). Cross-module via the public interface (constructor-inject `InventoryStockService` in order service). Build `lines` using the order line→`StockLine` mapping pinned in T019a (incl. the stable `sourceLineUid`).
- [ ] T024 [BE] [US1] Wire **invoice** service to call `applySale(INVOICE, invoiceUid, lines)` on invoice-finalize and `reverseSale(INVOICE/RETURN, …)` on void/credit-note (R3 default). Build `lines` using the invoice line→`StockLine` mapping pinned in T019a.
