...
import com.ampairs.file.config.AmpairsAwsProperties
...
/**
 * File metadata and storage coordination service backed by Spring Cloud AWS S3Template.
 */
@Service
@Transactional
    ...
    fun saveFile(
        ...
        return try {
            ...
            fileRepository.save(file).also {
                logger.info("File saved successfully using S3Template: name={}, key={}, size={}", name, objectKey, bytes.size)
            }
        } catch (e: Exception) {
    ...
    fun listFiles(folder: String): List<String> {
        return try {
            ...
            val listRequest = software.amazon.awssdk.services.s3.model.ListObjectsV2Request.builder()
                .bucket(ampairsAwsProperties.s3.defaultBucket)
                .prefix(prefix)
                .build()
