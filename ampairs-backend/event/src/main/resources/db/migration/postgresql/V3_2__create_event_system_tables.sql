...
COMMENT ON TABLE workspace_events IS 'Event store for workspace entity changes - supports offline sync and multi-device collaboration';
...
COMMENT ON TABLE device_sessions IS 'Tracks device connection status (online/away/offline) for real-time presence and multi-device sync';
