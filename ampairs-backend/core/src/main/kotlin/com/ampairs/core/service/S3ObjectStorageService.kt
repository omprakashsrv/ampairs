...
class S3ObjectStorageService(
    ...
    override fun getObjectMetadata(bucketName: String, objectKey: String): ObjectMetadata {
        return try {
            ...
            logger.error("Failed to get object metadata from S3: bucket={}, key={}, error={}", bucketName, objectKey, e.message, e)
    ...
    override fun generatePresignedUrl(bucketName: String, objectKey: String, expirationSeconds: Long): String {
        return try {
            ...
            logger.debug("Generated presigned URL for S3 object: bucket={}, key={}, expires={}s", bucketName, objectKey, expirationSeconds)
            presignedUrl.toString()
        } catch (e: Exception) {
            logger.error("Failed to generate presigned URL for S3: bucket={}, key={}, error={}", bucketName, objectKey, e.message, e)
    ...
    override fun objectExists(bucketName: String, objectKey: String): Boolean {
        return try {
            ...
            logger.warn("Error checking object existence in S3: bucket={}, key={}, error={}", bucketName, objectKey, e.message)
            false
        }
    }
    ...
    override fun copyObject(
        ...
        return try {
            ...
            logger.info("Object copied in S3: from={}:{} to={}:{}", sourceBucket, sourceKey, targetBucket, targetKey)
    ...
    override fun createBucketIfNotExists(bucketName: String) {
        try {
            ...
            try {
                val createRequest = CreateBucketRequest.builder().bucket(bucketName).build()
                s3Client.createBucket(createRequest)
                logger.info("S3 bucket created: {}", bucketName)
            } catch (ex: Exception) {
                logger.error("Failed to create S3 bucket: {}, error={}", bucketName, ex.message, ex)
                throw BucketCreationException("Failed to create S3 bucket: ${ex.message}", ex)
            }
        } catch (e: Exception) {
            logger.error("Error checking S3 bucket existence: {}, error={}", bucketName, e.message, e)
            throw BucketCreationException("Error checking S3 bucket: ${e.message}", e)
        }
    }
