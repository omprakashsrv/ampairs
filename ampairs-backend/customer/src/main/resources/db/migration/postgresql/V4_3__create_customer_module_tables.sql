...
COMMENT ON TABLE customer_groups IS 'Workspace-defined customer groupings with discount metadata';
...
COMMENT ON TABLE customer_types IS 'Workspace-defined customer categorisation master';
...
COMMENT ON TABLE master_states IS 'Global catalogue of geographic states for import into workspaces';
...
COMMENT ON TABLE state IS 'Workspace-specific state list optionally linked to master state catalogue';
...
COMMENT ON TABLE customer IS 'Workspace customer master records with billing and shipping metadata';
...
CREATE INDEX idx_customer_image_customer_uid ON customer_images(customer_uid);
CREATE INDEX idx_customer_image_workspace ON customer_images(owner_id);
CREATE INDEX idx_customer_image_primary ON customer_images(is_primary);
CREATE INDEX idx_customer_image_created ON customer_images(created_at);

COMMENT ON TABLE customer_images IS 'Customer imagery stored in object storage with workspace scoping';
