...
dependencies {
    ...
    // Master modules (public service interfaces only — never their repositories).
    implementation(project(mapOf("path" to ":cb_employee")))
    implementation(project(mapOf("path" to ":cb_store")))
    // Platform services reused the same way any ampairs module reuses them.
    ...
    // Scheduled batch jobs iterate the workspaces where cb-maintenance is enabled.
