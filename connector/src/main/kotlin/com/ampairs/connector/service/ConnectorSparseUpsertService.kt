 ...
 * This is SEPARATE from the global `/sync` upsert (FR-018a). Records a run for observability (FR-020).
 *
 * Writers are contributed by target modules via the core [ConnectorEntityWriter] SPI; when running in
 * the assembled service all modules' writers are injected here.
 */
@Service
    ...
    fun upsert(installationUid: String, entityType: String, rows: List<SparseUpsertRow>): List<SparseUpsertResult> {
        ...
        return synchronized(lock) {
            ...
            val results = rows.map { row ->
                ...
                val r = runCatching { writer.applySparse(row.refId, row.uid, present) }.getOrElse {
                    failed++
                    return@map SparseUpsertResult(row.refId, null, WriteOutcome.FAILED.name, emptyList(), it.message)
                }
                when (r.outcome) {
                ...
                SparseUpsertResult(row.refId, r.ampairsUid, r.outcome.name, r.appliedColumns, r.message)
            }
            run.processed = rows.size
            run.created = created
            run.updated = updated
            run.failed = failed
