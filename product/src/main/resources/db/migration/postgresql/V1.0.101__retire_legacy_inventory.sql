-- Retire the legacy flat `inventory` table (spec 014, T008) — PostgreSQL
...
SELECT 'whr_default_' || sub.owner_id, sub.owner_id, 'Default Warehouse', 'DEFAULT', 'WAREHOUSE', TRUE, TRUE, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
FROM (SELECT DISTINCT owner_id FROM inventory) sub
LEFT JOIN warehouse w ON w.owner_id = sub.owner_id AND w.is_default = TRUE
WHERE w.id IS NULL;
...
SELECT
    ...
    (SELECT w.uid FROM warehouse w WHERE w.owner_id = i.owner_id AND w.is_default = TRUE LIMIT 1),
    i.stock, i.stock, 0, 0, 0,
    i.unit_id, i.buying_price, i.selling_price, i.mrp, TRUE, i.created_at, i.updated_at
FROM inventory i
