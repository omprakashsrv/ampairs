...
interface TaxComponentRepository : CrudRepository<TaxComponent, Long>, PagingAndSortingRepository<TaxComponent, Long> {
    ...
    fun findByIsActive(isActive: Boolean): List<TaxComponent>
    fun findByIsActiveOrderByCalculationOrder(isActive: Boolean): List<TaxComponent>

    @Query("SELECT tc FROM tax_component tc WHERE tc.isActive = true AND tc.componentType IN :types ORDER BY tc.calculationOrder")
    fun findActiveByComponentTypesOrderByCalculationOrder(types: List<TaxComponentType>): List<TaxComponent>

    @Query("SELECT tc FROM tax_component tc WHERE tc.isActive = true AND (tc.applicableFrom IS NULL OR tc.applicableFrom <= :date) AND (tc.applicableTo IS NULL OR tc.applicableTo > :date)")
    fun findActiveForDate(date: LocalDateTime): List<TaxComponent>

    @Query("SELECT tc FROM tax_component tc WHERE tc.stateCodes LIKE %:stateCode% AND tc.isActive = true")
    fun findByStateCode(stateCode: String): List<TaxComponent>

    @Query("SELECT tc FROM tax_component tc WHERE tc.businessTypes LIKE %:businessType% AND tc.isActive = true")
    ...
    @Query("SELECT tc FROM tax_component tc WHERE tc.isCompound = true AND tc.isActive = true ORDER BY tc.calculationOrder")
    fun findCompoundComponents(): List<TaxComponent>

    @Query("SELECT DISTINCT tc.componentType FROM tax_component tc WHERE tc.isActive = true")
    fun findDistinctActiveComponentTypes(): List<TaxComponentType>

    @Query("SELECT tc FROM tax_component tc WHERE tc.defaultRate BETWEEN :minRate AND :maxRate AND tc.isActive = true")
